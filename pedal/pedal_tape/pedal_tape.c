...
void pedal_tape_on_adc_irq_fifo();
...
int main(void) {
    ...
    multicore_launch_core1(pedal_tape_core_1);
...
void pedal_tape_core_1() {
    ...
    /* ADC Settings */
    adc_init();
    adc_gpio_init(PEDAL_TAPE_ADC_0_GPIO); // GPIO26 (ADC0)
    adc_gpio_init(PEDAL_TAPE_ADC_1_GPIO); // GPIO27 (ADC1)
    adc_gpio_init(PEDAL_TAPE_ADC_2_GPIO); // GPIO28 (ADC2)
    adc_set_clkdiv(0.0f);
    ...
    irq_set_exclusive_handler(ADC_IRQ_FIFO, pedal_tape_on_adc_irq_fifo);
    ...
    pedal_tape_conversion_1_temp = PEDAL_TAPE_ADC_MIDDLE_DEFAULT;
    pedal_tape_conversion_2_temp = PEDAL_TAPE_ADC_MIDDLE_DEFAULT;
    pedal_tape_conversion_3_temp = PEDAL_TAPE_ADC_MIDDLE_DEFAULT;
    ...
    pedal_tape_is_outstanding_on_adc = true;
...
void pedal_tape_on_pwm_irq_wrap() {
    //uint32 from_time = time_us_32();
    uint16 conversion_1_temp = pedal_tape_conversion_1_temp;
    uint16 conversion_2_temp = pedal_tape_conversion_2_temp;
    uint16 conversion_3_temp = pedal_tape_conversion_3_temp;
    if (! pedal_tape_is_outstanding_on_adc) {
        pedal_tape_is_outstanding_on_adc = true;
        adc_select_input(0); // Ensure to Start from A0
        __dsb();
        __isb();
    ...
    } else if (output_1 < PEDAL_TAPE_PWM_OFFSET - PEDAL_TAPE_PWM_PEAK) {
        output_1 = PEDAL_TAPE_PWM_OFFSET - PEDAL_TAPE_PWM_PEAK;
    }
    int32 output_1_inverted = -mixed_1 + middle_moving_average;
    if (output_1_inverted > PEDAL_TAPE_PWM_OFFSET + PEDAL_TAPE_PWM_PEAK) {
        output_1_inverted = PEDAL_TAPE_PWM_OFFSET + PEDAL_TAPE_PWM_PEAK;
    } else if (output_1_inverted < PEDAL_TAPE_PWM_OFFSET - PEDAL_TAPE_PWM_PEAK) {
        output_1_inverted = PEDAL_TAPE_PWM_OFFSET - PEDAL_TAPE_PWM_PEAK;
    }
    ...
    pwm_clear_irq(pedal_tape_pwm_slice_num); // Seems Overlap IRQ Otherwise
    //pedal_tape_debug_time = time_us_32() - from_time;
...
void pedal_tape_on_adc_irq_fifo() {
    ...
    //printf("@pedal_tape_on_adc_irq_fifo 1 - adc_fifo_level: %d\n", adc_fifo_level);
    for (uint16 i = 0; i < adc_fifo_level; i++) {
        //printf("@pedal_tape_on_adc_irq_fifo 2 - i: %d\n", i);
        uint16 temp = adc_fifo_get();
        temp &= 0x7FFF; // Clear Bit[15]: ERR
        uint16 remainder = i % 3;
        if (remainder == 2) {
            pedal_tape_conversion_3_temp = temp;
        } else if (remainder == 1) {
            pedal_tape_conversion_2_temp = temp;
        } else if (remainder == 0) {
            pedal_tape_conversion_1_temp = temp;
        }
    }
    //printf("@pedal_tape_on_adc_irq_fifo 3 - adc_fifo_is_empty(): %d\n", adc_fifo_is_empty());
    adc_fifo_drain();
    pedal_tape_is_outstanding_on_adc = false;
}
