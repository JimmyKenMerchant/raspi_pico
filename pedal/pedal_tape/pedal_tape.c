...
int main(void) {
    ...
    gpio_put(PEDAL_TAPE_LED_GPIO, 1);
    /* Initialize PWM and Switch */
    pedal_pico_tape = util_pedal_pico_init(UTIL_PEDAL_PICO_PWM_1_GPIO, UTIL_PEDAL_PICO_PWM_2_GPIO);
    /* Initialize ADC */
    ...
    util_pedal_pico_process = pedal_pico_tape_process;
    /* Launch Core 1 */
    uint32* stack_pointer = (int32*)malloc(PEDAL_TAPE_CORE_1_STACK_SIZE);
    multicore_launch_core1_with_stack(util_pedal_pico_start, stack_pointer, PEDAL_TAPE_CORE_1_STACK_SIZE);
    while (true) {
        //printf("@main 1 - pedal_pico_tape_conversion_1 %08x\n", pedal_pico_tape_conversion_1);
        //printf("@main 2 - pedal_pico_tape_conversion_2 %08x\n", pedal_pico_tape_conversion_2);
        //printf("@main 3 - pedal_pico_tape_conversion_3 %08x\n", pedal_pico_tape_conversion_3);
