...
volatile util_pedal_pico* pedal_buffer;
...
void pedal_buffer_core_1() {
    /* PWM Settings */
    pedal_buffer = util_pedal_pico_init(PEDAL_BUFFER_PWM_1_GPIO, PEDAL_BUFFER_PWM_2_GPIO);
    irq_set_exclusive_handler(PWM_IRQ_WRAP, pedal_buffer_on_pwm_irq_wrap);
    irq_set_priority(PWM_IRQ_WRAP, 0xF0);
    pwm_set_chan_level(pedal_buffer->pwm_1_slice, pedal_buffer->pwm_1_channel, PEDAL_BUFFER_PWM_OFFSET);
    pwm_set_chan_level(pedal_buffer->pwm_2_slice, pedal_buffer->pwm_2_channel, PEDAL_BUFFER_PWM_OFFSET);
    /* ADC Settings */
...
void pedal_buffer_on_pwm_irq_wrap() {
    pwm_clear_irq(pedal_buffer->pwm_1_slice);
    //uint32 from_time = time_us_32();
    ...
    pedal_buffer_process(conversion_1, conversion_2, conversion_3);
    /* Output */
    pwm_set_chan_level(pedal_buffer->pwm_1_slice, pedal_buffer->pwm_1_channel, (uint16)pedal_buffer->output_1);
    pwm_set_chan_level(pedal_buffer->pwm_2_slice, pedal_buffer->pwm_2_channel, (uint16)pedal_buffer->output_1_inverted);
    //pedal_buffer_debug_time = time_us_32() - from_time;
...
void pedal_buffer_process(uint16 conversion_1, uint16 conversion_2, uint16 conversion_3) {
    pedal_buffer_conversion_1 = conversion_1;
    if (abs(conversion_2 - pedal_buffer_conversion_2) > UTIL_PEDAL_PICO_ADC_THRESHOLD) {
        pedal_buffer_conversion_2 = conversion_2;
        pedal_buffer_delay_time = (pedal_buffer_conversion_2 >> 7) << PEDAL_BUFFER_DELAY_TIME_SHIFT; // Make 5-bit Value (0-31) and Shift
    }
    if (abs(conversion_3 - pedal_buffer_conversion_3) > UTIL_PEDAL_PICO_ADC_THRESHOLD) {
        pedal_buffer_conversion_3 = conversion_3;
        pedal_buffer_noise_gate_threshold = (pedal_buffer_conversion_3 >> 7) * PEDAL_BUFFER_NOISE_GATE_THRESHOLD_MULTIPLIER; // Make 5-bit Value (0-31) and Multiply
    }
    ...
    int32 normalized_1 = (int32)pedal_buffer_conversion_1 - (int32)util_pedal_pico_adc_middle_moving_average;
    /**
    ...
    pedal_buffer->output_1 = util_pedal_pico_cutoff_biased(mixed_1 + (int32)util_pedal_pico_adc_middle_moving_average, PEDAL_BUFFER_PWM_OFFSET + PEDAL_BUFFER_PWM_PEAK, PEDAL_BUFFER_PWM_OFFSET - PEDAL_BUFFER_PWM_PEAK);
    pedal_buffer->output_1_inverted = util_pedal_pico_cutoff_biased(-mixed_1 + (int32)util_pedal_pico_adc_middle_moving_average, PEDAL_BUFFER_PWM_OFFSET + PEDAL_BUFFER_PWM_PEAK, PEDAL_BUFFER_PWM_OFFSET - PEDAL_BUFFER_PWM_PEAK);
}

void pedal_buffer_free() { // Free Except Object, pedal_buffer
    util_pedal_pico_stop((util_pedal_pico*)pedal_buffer);
    irq_remove_handler(PWM_IRQ_WRAP, pedal_buffer_on_pwm_irq_wrap);
    free((void*)pedal_buffer_delay_array);
    __dsb();
}
