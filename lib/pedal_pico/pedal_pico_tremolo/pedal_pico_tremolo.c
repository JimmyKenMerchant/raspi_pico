...
void pedal_pico_tremolo_set() {
    ...
    pedal_pico_tremolo_osc_speed = pedal_pico_tremolo_conversion_2 >> UTIL_PEDAL_PICO_ADC_FINE_SHIFT; // Make 6-bit Value (0-63)
    pedal_pico_tremolo_osc_is_negative = false;
    pedal_pico_tremolo_osc_start_threshold = (pedal_pico_tremolo_conversion_3 >> UTIL_PEDAL_PICO_ADC_FINE_SHIFT) * PEDAL_PICO_TREMOLO_OSC_START_THRESHOLD_MULTIPLIER; // Make 6-bit Value (0-63) and Multiply
...
void pedal_pico_tremolo_process(int32_t normalized_1, uint16_t conversion_2, uint16_t conversion_3, uint8_t sw_mode) {
    if (abs(conversion_2 - pedal_pico_tremolo_conversion_2) >= UTIL_PEDAL_PICO_ADC_FINE_THRESHOLD) {
        pedal_pico_tremolo_conversion_2 = conversion_2;
        pedal_pico_tremolo_osc_speed = pedal_pico_tremolo_conversion_2 >> UTIL_PEDAL_PICO_ADC_FINE_SHIFT; // Make 6-bit Value (0-63)
    }
    if (abs(conversion_3 - pedal_pico_tremolo_conversion_3) >= UTIL_PEDAL_PICO_ADC_FINE_THRESHOLD) {
    ...
    /* Get Oscillator */
    int32_t fixed_point_value_triangle_1 = util_pedal_pico_table_triangle_1[abs(pedal_pico_tremolo_osc_triangle_1_index) / PEDAL_PICO_TREMOLO_OSC_TRIANGLE_1_TIME_MULTIPLIER]; // Depending on osc_speed, the index value may have a negative value.
