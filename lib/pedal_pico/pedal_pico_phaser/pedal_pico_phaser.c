...
void pedal_pico_phaser_start() {
    ...
    pwm_set_chan_level(pedal_pico_phaser->pwm_2_slice, pedal_pico_phaser->pwm_2_channel, PEDAL_PICO_PHASER_PWM_OFFSET);
    /* Unique Settings */
    pedal_pico_phaser_set();
    /* Start */
    util_pedal_pico_start((util_pedal_pico*)pedal_pico_phaser);
    util_pedal_pico_sw_loop(PEDAL_PICO_PHASER_SW_1_GPIO, PEDAL_PICO_PHASER_SW_2_GPIO);
}
...
void pedal_pico_phaser_on_pwm_irq_wrap() {
    pwm_clear_irq(pedal_pico_phaser->pwm_1_slice);
    ...
    pedal_pico_phaser_process(conversion_1, conversion_2, conversion_3, util_pedal_pico_sw_mode);
    /* Output */
    pwm_set_chan_level(pedal_pico_phaser->pwm_1_slice, pedal_pico_phaser->pwm_1_channel, (uint16)pedal_pico_phaser->output_1);
    pwm_set_chan_level(pedal_pico_phaser->pwm_2_slice, pedal_pico_phaser->pwm_2_channel, (uint16)pedal_pico_phaser->output_1_inverted);
    //pedal_pico_phaser_debug_time = time_us_32() - from_time;
    //multicore_fifo_push_blocking(pedal_pico_phaser_debug_time); // To send a made pointer, sync flag, etc.
    __dsb();
}
...
void pedal_pico_phaser_process(uint16 conversion_1, uint16 conversion_2, uint16 conversion_3, uchar8 sw_mode) {
     ...
     */
     normalized_1 = (int32)(int64)((((int64)normalized_1 << 16) * (int64)pedal_pico_phaser_table_pdf_1[abs(util_pedal_pico_cutoff_normalized(normalized_1, PEDAL_PICO_PHASER_PWM_PEAK))]) >> 32); // Two 16-bit Decimal Parts Need 32-bit Shift after Multiplication to Get Only Integer Part
    /**
    ...
    pedal_pico_phaser->output_1 = util_pedal_pico_cutoff_biased(mixed_1 + (int32)util_pedal_pico_adc_middle_moving_average, PEDAL_PICO_PHASER_PWM_OFFSET + PEDAL_PICO_PHASER_PWM_PEAK, PEDAL_PICO_PHASER_PWM_OFFSET - PEDAL_PICO_PHASER_PWM_PEAK);
    pedal_pico_phaser->output_1_inverted = util_pedal_pico_cutoff_biased(-mixed_1 + (int32)util_pedal_pico_adc_middle_moving_average, PEDAL_PICO_PHASER_PWM_OFFSET + PEDAL_PICO_PHASER_PWM_PEAK, PEDAL_PICO_PHASER_PWM_OFFSET - PEDAL_PICO_PHASER_PWM_PEAK);
}
