...
void pedal_pico_distortion_process(int32_t normalized_1, uint16_t conversion_2, uint16_t conversion_3, uint8_t sw_mode) {
    if (abs(conversion_2 - pedal_pico_distortion_conversion_2) >= UTIL_PEDAL_PICO_ADC_FINE_THRESHOLD) {
        pedal_pico_distortion_conversion_2 = conversion_2;
    }
    if (abs(conversion_3 - pedal_pico_distortion_conversion_3) >= UTIL_PEDAL_PICO_ADC_FINE_THRESHOLD) {
