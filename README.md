...
* Caution that this project needs an analogue circuit to receive and output the audio signal, i.e., a DC bias on receiving, low-pass filters on outputting (in case of stereo outputting, this project outputs audio signals from two PWM channels). For reference, [I uploaded a schematic](assets/schematics/pedal_pico.pdf). This schematic applies a difference amplifier referencing [A Deeper Look into Difference Amplifiers by Harry Holt](https://www.analog.com/en/analog-dialogue/articles/deeper-look-into-difference-amplifiers.html). The pins layout of Pico is derived from [RP_Silicon_KiCad](https://github.com/HeadBoffin/RP_Silicon_KiCad). Note that the 3D image for KiCad is [https://github.com/ncarandini/KiCad-RP-Pico](https://github.com/ncarandini/KiCad-RP-Pico). As long as an Op Amp can sink electric current, the output voltage can be negative even if the power source is only positive. However, the single power LM358 makes a sink on 0V (logically) at minimum. On the balanced monaural with a single power Op Amp, the offset of the positive is risen to OFFSET + (OFFSET / 2), and the offset of the negative is fallen to OFFSET - (OFFSET / 2). In my experience on developing, the current consumption with a 9V battery (actual approx. 8.6V) is approx. 35mA. There are several single power Op Amps to be modified crossover distortion on changing between sourcing and sinking. However, LM358 since 1972 with bipolar junction transistors, has a simpler circuit than followers, i.e., you can treat issues on developing, e.g., an unintended resonance, noise, etc. I think its simplicity makes an advantages on a stability of a product. Note that Op Amps tends to make a relaxation oscillator unexpectedly by its parasitic capacitance, and it causes noise. Watch a circuit of an Op Amp, and you can see how it has complexity. The output impedance of the schematic is 1K ohms, that is a little higher than ones of state-of-the-art line outputs, because of the internal resistance of LM358 and a battery. The schematic have gain 4 at outputs (2 by the balanced circuit and 2 by the Op Amp), and these pedals gains up to 2 by correction. In Pico powered by 3.0V, PWM outputs between approx. from 0.75V (+-0.75V on balanced) to 2.25V (+-0.75 on balanced), but a pulse of PWM is reduced by a low-pass filter. I tested the output by a USB Audio Interface, UCA222/UCA202, which inputs voltage up to 2dBV = 1.26V at the positive peak without any potentiometer or any gain control. I applied a non-true-bypass (just normal bypass) to the foot switch in the schematic. These pedals uses a moving average to know the DC bias of the input. Besides, the true-bypass, that grounds circuit's input during turning off, may make popping noise at turning on. When you're turning off the non-true-byass in this schematic, 1M ohms resistance is set to the ground. This resistance is for DC biasing before the Op Amp. Logically in chaining pedals, the resistance at 1M ohms between Hot and Cold doesn't reduce sound quality at a low-impedance output from a buffer, but reduce sound quality at a high-impedance output from a guitar directly. I thought buffering on turning off, but there pedals are digital although the sound is not wrong. In case, I added a switch to change from any effect to a buffer, and vice versa. You can see there is no level control for the output in the schematic. Because of the digital circuit that would output voltage wave stably, there is no need to adjust the level to fit with ambient temperature. Besides, I considered of possible mechanical glitches and noise on the level POT. However, when you apply two Pico pedals on this project in serial, you may want to adjust the level of the first one. The best solution is to intermediate an active buffer between Pico pedals, and as an intermediator, LM386 is good enough. You can also make a passive attenuator by a POT, two RCA jacks and a plastic case.
...
* Sideband Reverb is the combination of the Sideband and the Reverb. ADC0 is for the audio input, ADC1 is for the mixing rate of the dry = current and the wet = delay (Dial 0 is the loudest volume), and ADC2 is for the room size (delay time). There are output modes. Switch-1 sets non-feedback (echo) mode. The mode of the Sideband is fixed to the non-oscillation mode.
